...
//READ ALL USERS
	$data = 'SELECT id, pseudo, prenom, nom, email, LEFT(password, 10) AS password, DATE_FORMAT(date_inscription, "%d/%m/%Y à %H:%i") AS date_inscription FROM membres';
...
if(isset($_GET['delete']) || isset($_POST['delete'])){
	$id = $_GET['delete'];
	$delete = true;
	$req = $bdd->query("DELETE FROM membres WHERE id =  $id");

	$_SESSION['delete'] = "</br>Un utilisateur a été supprimé avec succès";
}
...
if(isset($_POST['update'])){
	...
	$bdd->query("UPDATE membres SET pseudo = '$pseudo', prenom = '$prenom', nom = '$nom', email = '$email' WHERE id = $id");
